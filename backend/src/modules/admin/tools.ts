...
export async function adminToolsRoutes(app: FastifyInstance) {
  ...
  app.post('/admin/tools', { preHandler: requireAdmin }, async (request, reply) => {
    ...
    const existing = await prisma.tool.findUnique({ where: { key: parsed.data.key } });
    if (existing) {
  ...
  app.patch('/admin/tools/:id', { preHandler: requireAdmin }, async (request, reply) => {
    ...
    try {
      const tool = await prisma.tool.update({ where: { id: params.data.id }, data: body.data });
  ...
  app.delete('/admin/tools/:id', { preHandler: requireAdmin }, async (request, reply) => {
    const params = paramsSchema.safeParse(request.params);
    if (!params.success) {
      return reply.status(400).send({ error: 'invalid_params' });
    }
    try {
      await prisma.tool.delete({ where: { id: params.data.id } });
